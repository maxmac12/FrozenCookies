		...
		case 0:
			...
			switch (orderLvl) {
				...
				case 1: //Rigidel is already in diamond slot
					if(timeToRipe < 56.86 && Game.BuildingsOwned%10) {
						FrozenCookies.autoBuy = 0;
						rigiSell();
						Game.computeLumpTimes();
				...
				case 2: //Rigidel in Ruby slot,
					if(timeToRipe < 37.91 && Game.BuildingsOwned%10) {
						FrozenCookies.autoBuy = 0;
						rigiSell();
						Game.computeLumpTimes();
						autoRigidelAction.state = 2;
				...
				case 3: //Rigidel in Jade slot
					if (timeToRipe < 18.96 && Game.BuildingsOwned%10) {
						FrozenCookies.autoBuy = 0;
						rigiSell();
						Game.computeLumpTimes();
						autoRigidelAction.state = 2;
...
function autoFTHOFComboAction() {
	...
	if (M.magic == M.magicM) {
        ...
        if (M.magic >= Math.floor(FTHOF.costMin + FTHOF.costPercent*M.magicM)) { // Check if enough mana for FTHOF spell.
            if (!Game.hasBuff('Dragonflight')) {        // Don't run during DragonFlight since 'Click Frenzy' is removed from FTHOF pool while active.
                if (autoFTHOFComboAction.state != 2) {  // Don't evaluate if double FTHOF cast is in progress.
...
function autoFarmLumpsAction() 
{
    ...
    if (typeof autoFarmLumpsAction.state == 'undefined') {
        autoFarmLumpsAction.state = 'unlockgrimoire';
    }
    ...
    //     autoFarmLumpsAction.prevState = autoFarmLumpsAction.state;
    // }

    // // Log message indicating a state change.
    // if (autoFarmLumpsAction.state != autoFarmLumpsAction.prevState)
    // {
    //     console.log("State " + autoFarmLumpsAction.state);
    //     autoFarmLumpsAction.prevState = autoFarmLumpsAction.state;
    // }

    if (M)
    {
        // State machine for farming sugar lump golden cookies.
        switch (autoFarmLumpsAction.state)
        {
            case 'unlockgrimoire':  // Wait for Grimoire to be unlocked.
            ...
            case 'waitMana':  // Get enough mana to cast FTHOF.
                if (M.magicM < Math.floor(FTHOF.costMin + FTHOF.costPercent*M.magicM))
                {
                    // Not enough mana. Buy more wizard towers.
                    Game.Objects['Wizard tower'].buy(1);
                }
                else
                {
                    // Enough mana exists to cast FTHOF.
                    autoFarmLumpsAction.state = 'castSpell'
                }
                break;
            ...
            case 'castSpell':  // Cast FTHOF until sugar lump is cast.
                if (M.magic >= Math.floor(FTHOF.costMin + FTHOF.costPercent*M.magicM))
                {
                    if (nextSpellName(0) == SPELLNAME)
                    {
                        ...
                        autoFarmLumpsAction.prevAutoGC = FrozenCookies.autoGC; // Store auto click golden cookie state. 
                        FrozenCookies.autoGC = true;                           // Force golden cookie auto click.
                        autoFarmLumpsAction.state = 'clickGoldenCookie';       // Click spawned golden cookie.
                    }
                    else
                    {
            ...
            case 'clickGoldenCookie':  // Pop the spawned golden cookie.
                if (!goldenCookieLife())
                {
            ...
            case 'ascend':  // Ascend to look for another sugar lump.
                ...
                Game.OnAscend = 1;
                autoFarmLumpsAction.state = 'reincarnate';  // Reincarnate.
                break;

            case 'reincarnate':  // Reincarnate to look for another sugar lump.
                Game.Reincarnate(1);
                Game.ReincarnateTimer = Game.ReincarnateDuration + 1;  // Bypass reincarnation animation.
                autoFarmLumpsAction.state = 'showStopPrompt';          // Show the stop farm prompt.
                break;

            case 'showStopPrompt':  // Wait for reincarnation to complete.
                Game.CloseNotes();
                Game.Prompt('<div class="block">Do you want to stop farming?</div>',[['Yes!','FrozenCookies[\'autoFarmLumps\'] = 0; updateLocalStorage(); FCStart(); Game.ClosePrompt();'],['No','autoFarmLumpsAction.searching = false;']]);
                autoFarmLumpsAction.state = "unlockgrimoire";  // Check if a sugar lump is soon to be a FTHOF spell.  
                break;
...
function autoCookie() {
	...
	var fps_amounts = ['24', '30', '48', '60', '72', '88', '100', '120', '144', '200', '240', '300', '6000', '5', '10', '15'];
...
function FCStart() {
    ...
    if (FrozenCookies.autoFarmLumpsBot) {
        clearInterval(FrozenCookies.autoFarmLumpsBot);
        FrozenCookies.autoFarmLumpsBot = 0;
    }
    ...
    if (FrozenCookies.autoFarmLumps) {
        FrozenCookies.autoFarmLumpsBot = setInterval(autoFarmLumpsAction, FrozenCookies.frequency)
    }
    ...
    FrozenCookies.autoRigidelBot = setInterval(autoRigidelAction, FrozenCookies.frequency)
    }
